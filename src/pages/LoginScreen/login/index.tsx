...
const Login = () => {
  ...
  const { wechatLogin } = useWechat(navigation);
  const { t } = useTranslation()

  useEffect(() => {
    navigation.setOptions({
      headerLeft: () => <Image source={closeIcon} className='w-6 h-6 mr-4' />
    })
  }, [navigation])
      ...
      <View className="absolute left-16 right-16 bottom-0 h-64">
        ...
        <View className='mt-2 px-10 flex-row items-center justify-center'>
          {loginList.map((l, index) => {
            return (<TouchableOpacity key={index} onPress={() => mediaClick(l)}>
              <Image source={l.img} className='w-[24px] h-[24px]' />
            </TouchableOpacity>)
          })}
        </View>
